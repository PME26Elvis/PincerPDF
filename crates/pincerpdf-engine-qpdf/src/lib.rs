­r‡^Ñf¥–Ø¦{^¬yÊ'vÃ®¶›­#![forbid(unsafe_code)]
    ...
    fn bookmark_plan(
        ...
        for (prepared_source, input) in prepared.iter().zip(inputs) {
            let source_roots = if matches!(
                ...
                let capture = self.run_qpdf(
                    &[
                        ...
                        prepared_source.path.as_os_str().to_os_string(),
                    ],
                    vec![
                        "--json=2".to_owned(),
            ...
            let contributed_pages = input.pages.len()
                + usize::from(add_blank_page_if_odd && input.pages.len() % 2 == 1);
