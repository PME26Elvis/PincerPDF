...
- `application-shell-desktop.png` at 1440 Ã— 1278,
- `application-shell-compact.png` at 390 Ã— 3199.
...
The corrected run opened `http://tauri.localhost/`, loaded the JavaScript/WASM, exposed `window.__TAURI__`, and crossed the real `merge_engine_status` and `cancel_merge` command bridge. The suite passed 3/3 scenarios in 2.6â€“4.8 seconds of test time and nine seconds end to end:
...
The production-like binary still has only `core:default`; no WDIO Rust plugin, guest script or `wdio:*` capability is shipped. The retained native screenshot is 3600 Ã— 2110:
