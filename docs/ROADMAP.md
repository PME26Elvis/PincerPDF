...
- **P4.4 — Document-level bookmarks: In progress.** Explicit discard and one-entry-per-document policies cross the UI, desktop and engine boundaries. Real Windows QPDF/MuPDF, native-command, browser, production WebView2 and real system-dialog evidence is green; Linux compatibility remains.
