...
| L5 Packaged Linux E2E | Tauri commands/events, real filesystem and Linux ports | tauri-driver/WebDriverIO + Xvfb | Boundary flows and RC |
| L6 Visual/motion | Static baselines, checkpoint submissions and motion frames | Playwright screenshots, ffmpeg, Vision review | Per milestone/RC |
...
The workbook contains all 229 upstream test files and 907 detected test methods. Each row receives a target Rust test ID/layer and disposition. `NoHeadless` tests are not skipped; they are candidates for browser component tests, packaged Linux E2E or visual checkpoints.
